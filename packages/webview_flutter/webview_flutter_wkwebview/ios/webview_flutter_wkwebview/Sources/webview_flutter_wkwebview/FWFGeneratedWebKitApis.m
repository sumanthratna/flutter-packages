...
void SetUpFWFWKHttpCookieStoreHostApi(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FWFWKHttpCookieStoreHostApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.webview_flutter_wkwebview.WKHttpCookieStoreHostApi.getAllCookies"
        binaryMessenger:binaryMessenger
        codec:FWFWKHttpCookieStoreHostApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(getAllCookiesForStoreWithIdentifier:completion:)], @"FWFWKHttpCookieStoreHostApi api (%@) doesn't respond to @selector(getAllCookiesForStoreWithIdentifier:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSInteger arg_identifier = [GetNullableObjectAtIndex(args, 0) integerValue];
        [api getAllCookiesForStoreWithIdentifier:arg_identifier completion:^(NSArray<FWFNSHttpCookieData *> *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
